...
const OrderCreateModal: React.FC<OrderCreateModalProps> = ({ open, onClose, onSuccess }) => {
  ...
  // เพิ่ม useEffect สำหรับจัดการเมื่อปิด modal หรือโหลดหน้าใหม่
  useEffect(() => {
    const handleBeforeUnload = async () => {
      // ลบ minimal contract ถ้ามี เมื่อปิดหน้าเว็บ
      if (autoGeneratedContractId) {
        try {
          await deleteMinimalContract(autoGeneratedContractId);
          console.log('ลบ minimal contract เมื่อปิดหน้าเว็บ:', autoGeneratedContractId);
        } catch (error) {
          console.error('ไม่สามารถลบ minimal contract เมื่อปิดหน้าเว็บได้:', error);
        }
      }
    };
  ...
  // เพิ่ม useEffect สำหรับลบ minimal contract ที่ค้างอยู่เมื่อโหลดหน้าใหม่
  useEffect(() => {
    const cleanupPendingContracts = async () => {
      ...
      if (pendingContractId) {
        try {
          ...
          console.log('ลบ minimal contract ที่ค้างอยู่:', pendingContractId);
          localStorage.removeItem('pendingMinimalContractId');
        } catch (error) {
    ...
    // ลบ minimal contract ที่ค้างอยู่เมื่อโหลดหน้าใหม่
  ...
  const handleContractFileTypeChange = async (type: 'upload' | 'auto') => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (isCashPurchase) {
      ...
      if (contractFileType === 'upload') {
        ...
        if (!autoGeneratedContractId) {
          toast.error('ไม่พบเลขคำสั่งซื้อ กรุณาสร้างไฟล์ PDF ใหม่');
          setIsSubmitting(false);
          return;
        }
